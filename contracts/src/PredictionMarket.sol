...
contract PredictionMarket is AccessControl, ReentrancyGuard {
    ...
    function buy(
        ...
        // Fixed price: 1 token -> 1 share, fee taken from input
        uint256 fee = (amountIn * m.feeBps) / 10_000;
        uint256 sharesOut = amountIn - fee;
        ...
        emit Bought(marketId, msg.sender, outcome, amountIn, sharesOut, fee);
    }
    ...
    function withdrawFees(address to, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        // Fees are part of contract's token balance; limit by accrued amount
    ...
    // For MVP we track a single global accrued amount by summing per-market field on updates.
    // To withdraw, we decrease proportionally from markets until amount is covered.
